...
const canBeCapturedBySomeEnemyEnPassant = (squareOfFriendlyPawn, friendlies, enemies, lastMove) => {
    if (!lastMove || squareOfFriendlyPawn !== lastMove[1])
        return false;
    const srcPos = util.key2pos(lastMove[0]), destPos = util.key2pos(lastMove[1]), piece = friendlies.get(lastMove[1]);
...
const isPathClearEnoughOfEnemiesForPremove = (ctx) => {
    ...
    const enemyPawnDests = [
        ...
        ...[squareAbove, util.squareShiftedVertically(squareAbove, enemyStep)].filter(s => canEnemyPawnAdvanceToSquare(enemySquare, s, ctx)),
    ];
